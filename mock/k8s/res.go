...
func ApplyManifest(phaseName, kubectlHost string, paramList [][]any, logger logx.Logger) (bool, error) {
	// 1 - get parameters
	// check
	if len(paramList) < 1 || len(paramList[0]) == 0 {
		return false, fmt.Errorf("local:%s > list of manifests not properly provided in paramList", kubectlHost)
	}
	// 11 - slice to managed
	slice, err := filex.GetVarStructFromYaml[kubectl.SliceResource](paramList[0])
	if err != nil {
