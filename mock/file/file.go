...
// Description: create a file to a set of nodes from hostname
func Create(phaseName, hostName string, paramList [][]any, logger logx.Logger) (bool, error) {
	...
	filePath := fmt.Sprint(paramList[1][0])
	...
	for _, node := range nodeList {
		...
		go func(oneItem string) { // create as many goroutine (that will run concurrently) as item AND pass the item as an argument
			...
			logger.Infof("↪ (%s) %s/%s > ongoing", phaseName, hostName, oneItem)
			_, grErr := lfile.Create(hostName, node, filePath, logger) // the code to be executed by the goroutine
			if grErr != nil {
				logger.Errorf("(%s) %s/%s > %v", phaseName, hostName, oneItem, grErr) // send goroutines error if any into the chanel
				// send goroutines error if any into the chanel
				errChNode <- fmt.Errorf("%w", grErr)
...
// Description: Echo a string into a file to a set of nodes from hostname
func AddString(phaseName, hostName string, paramList [][]any, logger logx.Logger) (bool, error) {
	...
	for _, node := range nodeList {
		...
		go func(oneItem string) { // create as many goroutine (that will run concurrently) as item AND pass the item as an argument
			...
			logger.Infof("↪ (%s) %s/%s > ongoing", phaseName, hostName, oneItem)
			_, grErr := lfile.AddStringOnce(hostName, node, filePath, logger) // the code to be executed by the goroutine
			if grErr != nil {
				logger.Errorf("(%s) %s/%s > %v", phaseName, hostName, oneItem, grErr) // send goroutines error if any into the chanel
				// send goroutines error if any into the chanel
				errChNode <- fmt.Errorf("%w", grErr)
