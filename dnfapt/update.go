...
func updateSingleVmOsApp(logger logx.Logger, vmName string, requiredPackages []string) (string, error) {
	...
	logger.Debugf(" > osFamily: %s\n", osFamily)
	logger.Debugf(" > Required packages: %v\n", requiredPackages)
	// ... your logic to install the packages here ...
	...
	for _, t := range targets {
		...
		tasks = append(tasks, func() error {
			if _, err := updateSingleVmOsApp(logger, vmCopy.Name(), requiredPackages); err != nil {
				logger.Errorf("🅣 Failed to update VM %s: %v", vmCopy.Name(), err)
				return err
			}

			logger.Infof("🅣 VM %s updated successfully", vmCopy.Name())
			return nil
		})
	}
